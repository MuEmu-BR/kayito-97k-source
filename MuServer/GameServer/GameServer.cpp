				...
				case IDC_LIST1:
				{
					switch (HIWORD(wParam))
					{
						case LBN_SELCHANGE:
						{
							...
							if (gObj[i].Name[0] == 0)
							{
								SetDlgItemText(hDlg, IDC_EDIT3, "SELECT CHARACTER");
							...
							{
								SetDlgItemText(hDlg, IDC_EDIT3, gObj[i].Name);

								SetDlgItemText(hDlg, IDC_EDIT4, gMapManager.GetMapName(gObj[i].Map));
