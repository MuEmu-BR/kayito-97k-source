...
void CObjectManager::ObjectStateProc(LPOBJ lpObj, int MessageCode, int aIndex, int SubCode)
{
	...
	switch (MessageCode)
	{
		...
		case 10:
		{
			...
			{
				gAttack.Attack(lpObj, &gObj[aIndex], 0, 0, 0, SubCode, 0);
			}
...
void CObjectManager::ObjectStateAttackProc(LPOBJ lpObj, int MessageCode, int aIndex, int SubCode1, int SubCode2)
{
	...
	switch (MessageCode)
	{
		case 50:
		{
			...
			{
				gAttack.Attack(lpObj, &gObj[aIndex], gSkillManager.GetSkill(lpObj, SubCode1), 0, 0, 0, 0);
			}
		...
		case 52:
		{
			if (lpObj->Live != 0 && (SubCode1 == 0 || gSkillManager.GetSkill(lpObj, SubCode1) != 0))
			{
				gAttack.Attack(lpObj, &gObj[aIndex], gSkillManager.GetSkill(lpObj, SubCode1), 0, 0, 0, SubCode2);
			}
		...
		case 53:
		{
			if (lpObj->Live != 0 && (SubCode1 == 0 || gSkillManager.GetSkill(lpObj, SubCode1) != 0))
			{
				gAttack.Attack(lpObj, &gObj[aIndex], gSkillManager.GetSkill(lpObj, SubCode1), 1, 0, 0, 0);
			}
		...
		case 54:
		{
			if (lpObj->Live != 0 && (SubCode1 == 0 || gSkillManager.GetSkill(lpObj, SubCode1) != 0))
			{
				gAttack.Attack(lpObj, &gObj[aIndex], gSkillManager.GetSkill(lpObj, SubCode1), 1, 0, 0, SubCode2);
			}
...
void CObjectManager::CharacterUpdateMapEffect(LPOBJ lpObj)
{
	for (int i = 0; i < MAX_EFFECT_LIST; i++)
	{
		...
		gEffectManager.GCEffectStateSend(lpObj, 0, lpEffect->m_index);

		gEffectManager.AddEffect(lpObj, 0, lpEffect->m_index, lpEffect->m_count, lpEffect->m_value[0], lpEffect->m_value[1], lpEffect->m_value[2], lpEffect->m_value[3]);
	}
}
...
void CObjectManager::CharacterCalcAttribute(int aIndex)
{
	...
	if (lpObj->ArmorSetBonus != 0 && (Level11Count + Level10Count) >= 5)
	{
		if (Level11Count == 5)
		{
			lpObj->Defense += (lpObj->Defense * 10) / 100;
		}
		else if (Level10Count == 5 || (Level10Count + Level11Count) == 5)
		{
