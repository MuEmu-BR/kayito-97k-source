...
void gObjMonsterDieGiveItem(LPOBJ lpObj, LPOBJ lpTarget)
{
	...
	if ((DropIndex = gMonsterManager.GetMonsterItem(DropLevel, ExcItemDrop)) >= GET_ITEM(0, 0) && (GetLargeRand() % ((lpObj->ItemRate == 0) ? 1 : lpObj->ItemRate)) < ItemDropRate)
	{
		...
		gItemOptionRate.GetItemLevelOption(((ExcItemDrop == 0) ? 0 : 1), &ItemLevel);

		gItemOptionRate.GetItemSkillOption(((ExcItemDrop == 0) ? 0 : 1), &ItemSkillOption);

		gItemOptionRate.GetItemLuckOption(((ExcItemDrop == 0) ? 0 : 1), &ItemLuckOption);

		gItemOptionRate.GetItemAddOption(((ExcItemDrop == 0) ? 0 : 1), &ItemAddOption);

		gItemOptionRate.GetItemExceOption(((ExcItemDrop == 0) ? 0 : 1), &ItemExceOption);
...
void gObjMonsterBeattackRecv(BYTE* aRecv, int aIndex)
{
	...
	for (int n = 0; n < lpMsg->count; n++)
	{
		...
		gAttack.Attack(&gObj[aIndex], &gObj[bIndex], lpSkill, 1, 1, 0, 0);
	}
}
...
void gObjMonsterStateProc(LPOBJ lpObj, int code, int aIndex, int SubCode)
{
	switch (code)
	{
		...
		case 55:
		{
			if (gObj[aIndex].Live != 0)
			{
				gAttack.Attack(lpObj, &gObj[aIndex], 0, 0, 0, 0, 0);
			}
	...
	if (lpObj->ActionState.Attack == 1)
	{
		if (CMonsterSkillManager::CheckMonsterSkill(lpObj->Class, 0))
		{
			...
			if (GetLargeRand() % 4 == 0)
			{
				...
				gAttack.Attack(lpObj, &gObj[lpObj->TargetNumber], NULL, FALSE, 0, 0, 0);
			}
			else
			{
