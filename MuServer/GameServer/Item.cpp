...
void CItem::Convert(int index, BYTE SkillOption, BYTE LuckOption, BYTE AddOption, BYTE ExceOption)
{
	...
	else
	{
		...
		this->m_SpecialIndex[SPECIAL_OPTION1] = ((ItemInfo.Skill == SKILL_NONE) ? this->m_SpecialIndex[SPECIAL_OPTION1] : ItemInfo.Skill);
	...
	else
	{
		...
		this->m_SpecialValue[SPECIAL_OPTION3] = this->m_SpecialValue[SPECIAL_OPTION3] * this->m_AddOption;
...
void CItem::Value()
{
	...
	else
	{
		...
		if (((this->m_Index / MAX_ITEM_TYPE) == 12 && this->m_Index > GET_ITEM(12, 6)) || (this->m_Index / MAX_ITEM_TYPE) == 13 || (this->m_Index / MAX_ITEM_TYPE) == 15)
		{
			...
			price = ((this->m_SpecialIndex[SPECIAL_OPTION3] == ITEM_OPTION_ADD_HP_RECOVERY_RATE) ? (price + (price * this->m_AddOption)) : price);
		}
		else
		{
			switch (this->m_Level)
