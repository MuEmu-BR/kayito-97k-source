...
int CEffectManager::GenerateEffectList(LPOBJ lpObj, BYTE* lpMsg, int* size)
{
	...
	for (int n = 0; n < MAX_EFFECT_LIST; n++)
	{
		if (lpObj->Effect[n].IsEffect() != false)
		{
			...
			if (this->GetEffectState(lpObj->Effect[n].m_index, &result) != false)
			{
				count |= result;
			}
		}
...
bool CEffectManager::GetEffectState(BYTE effect, WORD* result)
{
	switch (effect)
	{
		case EFFECT_GREATER_DAMAGE:
		{
			(*result) = EFFECT_STATE_GREATER_DAMAGE;
		...
		case EFFECT_GREATER_DEFENSE:
		{
			(*result) = EFFECT_STATE_GREATER_DEFENSE;
		...
		case EFFECT_MANA_SHIELD:
		{
			(*result) = EFFECT_STATE_MANA_SHIELD;
		...
		case EFFECT_GREATER_LIFE:
		{
			(*result) = EFFECT_STATE_GREATER_LIFE;
		...
		case EFFECT_FIRE_SLASH:
		{
			(*result) = EFFECT_STATE_FIRE_SLASH;
...
void CEffectManager::GCEffectStateSend(LPOBJ lpObj, BYTE state, BYTE effect)
{
	...
	if (this->GetEffectState(effect, &pMsg.effect) == false)
	{
		return;
	}
