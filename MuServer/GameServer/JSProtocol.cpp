...
void JGConnectAccountRecv(SDHP_CONNECT_ACCOUNT_RECV* lpMsg)
{
	...
	LogAdd(LOG_USER, "[ObjectManager][%d] AddAccountInfo (%s)", lpMsg->index, gObj[lpMsg->index].Account);

	gLog.Output(LOG_CONNECT, "[ObjectManager][%d] AddAccountInfo (%s)", lpMsg->index, gObj[lpMsg->index].Account);
}
...
void JGAccountAlreadyConnectedRecv(SDHP_ACCOUNT_ALREADY_CONNECTED_RECV* lpMsg)
{
	...
	{
		gNotice.GCNoticeSend(lpMsg->index, 1, gMessage.GetTextMessage((361), gObj[lpMsg->index].Lang));
	}
}
