...
bool CAttack::Attack(LPOBJ lpObj, LPOBJ lpTarget, CSkill* lpSkill, bool send, BYTE flag, int damage, int count)
{
#pragma region ATTACK_CHECK
	...
	{
		if (lpObj->Guild->WarType == 1 && gBattleSoccer.GetBattleSoccerGoalMove(0) == 0)
		{
			return true;
		}
	}
...
void CAttack::DamageSprite(LPOBJ lpObj, int damage)
{
	...
	if (lpItem->m_Durability < 1)
	{
		...
		if (lpObj->Map == MAP_ICARUS && lpObj->Inventory[7].IsItem() == false && lpItem->m_Index == GET_ITEM(13, 3))
		{
...
void CAttack::CGAttackRecv(PMSG_ATTACK_RECV* lpMsg, int aIndex)
{
	...
	this->Attack(lpObj, lpTarget, 0, 0, 0, 0, 0);
}