...
void CGuild::DGGuildMemberJoinRecv(SDHP_GUILD_MEMBER_JOIN_RECV* lpMsg)
{
	...
	if (lpMsg->result == 1)
	{
		lpGuild = gGuildManager.AddMember(lpMsg->GuildName, lpMsg->Name, lpMsg->index, -1, GUILD_STATUS_MEMBER, lpMsg->server);
	}
...
void CGuild::DGGuildInfoRecv(SDHP_GUILD_INFO_RECV* lpMsg)
{
	...
	for (int n = 0; n < lpMsg->count; n++)
	{
		...
		if (gGuildManager.AddMember(lpMsg->GuildName, lpInfo->Name, -1, lpMsg->count, lpInfo->status, lpInfo->server) == 0)
		{
			break;
		}
	}
...
void CGuild::GuildWarRequestSend(int aIndex, char* GuildName, int type)
{
	...
	if (lpObj->Guild->WarState == 1 || lpObj->Guild->WarDeclareState == 1)
	{
	...
	if (lpInfo->WarState == 1 || lpInfo->WarDeclareState == 1)
	{
		this->GCGuildWarRequestResultSend(aIndex, 4);
